...
export function DefaultToolRender({status, name = "", args, result}: ToolCallProps) {
  ...
  const classes = {
  container:
    "bg-[#1a1a2a] rounded-xl overflow-hidden w-full border-2 border-[#2e2e3f] shadow-md transition-all duration-200 hover:shadow-lg my-1",
  header:
    "p-4 flex items-center cursor-pointer group bg-[#232332] border-b border-[#33334a]",
  title:
    "text-gray-100 font-semibold overflow-hidden text-ellipsis",
  statusContainer:
    "ml-auto flex items-center gap-2",
  statusText:
    "text-xs text-gray-400 font-medium mr-1",
  content:
    "px-5 pb-5 pt-3 text-gray-200 font-mono text-xs",
  section:
    "mb-4",
  sectionTitle:
    "text-gray-400 text-xs uppercase tracking-wider mb-2 font-sans font-bold",
  codeBlock:
    "whitespace-pre-wrap max-h-[200px] overflow-auto text-gray-100 bg-[#2b2b3d] p-3 rounded border border-[#3d3d50]",
  chevron: {
    base: "text-gray-400 mr-2 transition-transform duration-200",
    open: "rotate-90",
  ...
  const statusColors = {
    ...
    executing: "bg-orange-500 shadow-orange-500/40"
  };
